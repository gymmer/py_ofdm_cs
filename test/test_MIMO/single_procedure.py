...
''' 发送端 '''
bits_A,diagram_A,x = sender(pos_A,N,Ncp,M,Nt,Nr,modulate)

''' 信道传输 '''
h_ab,H_ab,y_b = transmission(x,SNR,L,K,N,Ncp,M,Nt,Nr)

''' 理想条件下的信道估计'''
# 合法用户确切知道发送端导频
h_lx,H_lx,bits_lx,diagram_lx = receiver(y_b,pos_A,'CS',L,K,N,Ncp,M,Nt,Nr,modulate)

''' 接收端 信道估计'''
h_cs,H_cs,bits_cs,diagram_cs = receiver(y_b,pos_B,'CS',L,K,N,Ncp,M,Nt,Nr,modulate)
 
''' 窃听信道 '''
h_ae,H_ae,y_e = transmission(x,SNR,L,K,N,Ncp,M,Nt,Nr)

''' 非法用户 '''
h_eva,H_eva,bits_eva,diagram_eva = receiver(y_e,pos_E,'CS',L,K,N,Ncp,M,Nt,Nr,modulate)
