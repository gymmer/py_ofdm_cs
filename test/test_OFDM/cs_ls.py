...
for i in range(gro_num):
    for j in range(SNR_num):
        ...
        h_cs,H_cs,bits_cs,diagram_cs = receiver(y_b,L,K,N,Ncp,pos_A,modulate_type,'CS')
        h_ae,H_ae,y_e = transmission(x,L,K,N,Ncp,SNR[j])
        h_eva,H_eva,bits_eva,diagram = receiver(y_e,L,K,N,Ncp,pos_E,modulate_type,'CS')
        ...
        bits_A,diagram_A,x = sender(N,Ncp,pos_A,modulate_type)
        h_ab,H_ab,y_b = transmission(x,L,K,N,Ncp,SNR[j])
        h_ls,H_ls,bits_ls,diagram_ls = receiver(y_b,L,K,N,Ncp,pos_B,modulate_type,'LS')
        h_ae,H_ae,y_e = transmission(x,L,K,N,Ncp,SNR[j])
        h_eva,H_eva,bits_eva,diagram = receiver(y_e,L,K,N,Ncp,pos_E,modulate_type,'LS')
...
plt.plot(SNR,CS_MSE,'ko-',label='CS(Random, P=%s)'%(P))
plt.plot(SNR,LS_MSE,'k^:',label='LS(Even, P=%s)'%(len(pos_ls)))
plt.xlabel('SNR(dB)')
plt.ylabel('MSE(dB)')
plt.title('MSE')
plt.legend()
...
plt.figure(figsize=(8,5))
plt.semilogy(SNR,CS_BER,'ko-',label='CS(Random, P=%s)'%(P))
plt.semilogy(SNR,LS_BER,'k^:',label='LS(Even, P=%s)'%(len(pos_ls)))
plt.xlabel('SNR(dB)')
plt.ylabel('BER')
plt.title('BER')
plt.legend()
...
plt.figure(figsize=(8,5))
plt.plot(SNR,CS_SC,'ko-',label='CS(Random, P=%s)'%(P))
plt.plot(SNR,LS_SC,'k^:',label='LS(Even, P=%s)'%(len(pos_ls)))
plt.xlabel('SNR(dB)')
