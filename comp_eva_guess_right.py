...
for i in range(gro_num):
    for j in range(P):
        ...
        ''' 根据RSSI产生导频图样'''
        pos_A,pos_B,pos_E = agreement(1,2,P)
        ...
        ''' 接收端 信道估计'''
        h_cs,H_cs,h_ls,H_ls = receiver(Y,W,L,N,P,K,pos_B)
        
        ''' 非法用户 '''
        # 非法用户随机猜测导频位置。与发送端的导频图样pos_A相比，非法用户猜对了j个,j取值[0,P)
        h_eva,H_eva = receiver_eva(Y,W,N,K,P,pos_A,'%d'%(j))
...
for i in range(gro_num):
    for j in range(SNR_num):
        ...
        ''' 非法用户 '''
        h_eva_r, H_eva_r  = receiver_eva(Y,W,N,K,P,pos_E,'from_pos')
        h_eva_2, H_eva_2  = receiver_eva(Y,W,N,K,P,pos_A,'2')
        h_eva_10,H_eva_10 = receiver_eva(Y,W,N,K,P,pos_A,'10')
        h_eva_20,H_eva_20 = receiver_eva(Y,W,N,K,P,pos_A,'20')
        h_eva_25,H_eva_25 = receiver_eva(Y,W,N,K,P,pos_A,'25')
        h_eva_30,H_eva_30 = receiver_eva(Y,W,N,K,P,pos_A,'30')
        h_eva_35,H_eva_35 = receiver_eva(Y,W,N,K,P,pos_A,'35')
