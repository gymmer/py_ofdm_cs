...
CS_Pe   = zeros((gro_num,SNR_num,right_num))
eva_Pe  = zeros((gro_num,SNR_num,right_num))
...
for i in range(gro_num):
    for j in range(SNR_num):
        for r in range(right_num):
            ...
            CS_Pe[i,j,r]   = BMR(bits_A,bits_cs)
            eva_Pe[i,j,r]  = BMR(bits_A,bits_eva)
            SC[i,j,r]      = SecCap(CS_Pe[i,j,r],eva_Pe[i,j,r])
...
plt.figure(figsize=(8,5))
plt.semilogy(SNR,CS_Pe[:,0], 'g*-.', label='Valid user')
plt.semilogy(SNR,eva_Pe[:,0],'r^--',label='EVA(guess %d right)'%(right[0]))
plt.semilogy(SNR,eva_Pe[:,1],'y<-.',label='EVA(guess %d right)'%(right[1]))
plt.semilogy(SNR,eva_Pe[:,2],'c>:', label='EVA(guess %d right)'%(right[2]))
plt.semilogy(SNR,eva_Pe[:,3],'bv-.', label='EVA(guess %d right)'%(right[3]))
plt.semilogy(SNR,eva_Pe[:,4],'ms--',label='EVA(guess %d right)'%(right[4]))
plt.xlabel('SNR(dB)')
plt.ylabel('Probability')
plt.title('Pe of evasdropper by random guessing')
plt.legend()
