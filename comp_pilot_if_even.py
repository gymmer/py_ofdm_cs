...
for i in range(gro_num):
    for j in range(SNR_num):
        ...
        h_ls,H_ls,bits_ls = receiver(y,L,K,N,Ncp,pos_B,modulate_type,'LS','from_pos')
        
        ''' 非法用户 '''
        h_eva,H_eva,bits_eva = receiver(y,L,K,N,Ncp,pos_E,modulate_type,'CS','from_pos')
...
for i in range(gro_num):
    for j in range(SNR_num):
        ...
        ''' 发送端 '''
        bits_A,x = sender(N,Ncp,pos,modulate_type)
        
        ''' 信道传输 ''' 
        h,H,W,y = transmission(x,L,K,N,Ncp,SNR[j])
        
        ''' 接收端 信道估计''' 
        h_cs,H_cs,bits_cs = receiver(y,L,K,N,Ncp,pos,modulate_type,'CS','from_pos')
        h_ls,H_ls,bits_ls = receiver(y,L,K,N,Ncp,pos,modulate_type,'LS','from_pos')
        
        ''' 非法用户 '''
        # 均匀放置导频时，非法用户可以很容易猜到导频图样。假设非法用户猜到了每5个插入一个导频
        # 这个时候，非法用户的重构性能，和基于CS的合法用户的重构性能，是完全一样的
        h_eva,H_eva,bits_eva = receiver(y,L,K,N,Ncp,pos,modulate_type,'CS','from_pos')
